...
public enum HairLossType {
    ...
    private static final Map<Integer, HairLossType> TYPES = Stream.of(HairLossType.values()).collect(Collectors.toMap(
                                                                      HairLossType::getTypeNumber,
                                                                      type -> type));

    public static HairLossType getType(String type) {

        return TYPES.get(Integer.parseInt(type.split("_")[1]));
